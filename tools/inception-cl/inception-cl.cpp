...
static std::error_code loadBinary(StringRef FileName) {
  ...
  // ErrorOr<Module*> moduleOrErr = getLazyBitcodeModule(fileOrErr.get(),
  // context);
  ...
  std::cout << " Name: " << old_module.get()->getName().str() << std::endl;
  std::cout << " Target triple: " << old_module.get()->getTargetTriple()
            << std::endl;
...
///===---------------------------------------------------------------------===//
/// runLoadBinaryCommand   - Loads an executable by reading a file
///
/// @param Executable - The executable under analysis.
///
static void runLoadBinaryCommand(std::vector<std::string> &CommandLine) {
  StringRef FileName;
  if (CommandLine.size() >= 2) FileName = CommandLine[1];
...
static void runSectionsCommand(std::vector<std::string> &CommandLine) {
  ...
  for (object::section_iterator si = DAS->getExecutable()->section_begin(),
                                se = DAS->getExecutable()->section_end();
       si != se; ++si) {
    if (error(ec)) return;
    StringRef Name;
...
static void dumpELFRelocSymbols(const object::ELFObjectFile<ELFT> *elf,
  ...
  for (object::relocation_iterator ri = CurrentSection.relocation_begin();
       ri != CurrentSection.relocation_end(); ++ri) {
    if (error(ec)) return;
    uint64_t Addr = 0;
    uint64_t SectAddr = 0;
    uint64_t Offset = 0;
    ...
    if (error(ri->getAddress(Addr))) continue;
    // if (error(ri->getOffset(Offset))) // FIXME: Maybe getOffset only
    //  continue;                       // works for a certain reloc type?
    if (error(ri->getType(Type))) continue;
    if (error(ri->getTypeName(TypeName))) continue;
    if (error(ri->getValueString(Value))) continue;

    object::SectionRef Section = DAS->getSectionByAddress(Addr);
    SectAddr = Section.getAddress();

    const char *Fmt;
    Fmt = elf->getBytesInAddress() > 4 ? "%016" PRIx64 : "%08" PRIx64;
    SmallVectorImpl<char>::iterator ti = TypeName.begin();
    outs() << format(Fmt, Addr) << " " << format(Fmt, Offset)
           << " "  // FIXME: Should be Info section
           << ti << "\t" << format(Fmt, Offset)
           << " "  // FIXME: Should be Symbol Value
           << Name << "\n";
  }
}
...
static void dumpELFSymbols(const object::ELFObjectFile<ELFT> *elf,
  ...
  for (std::vector<FractureSymbol *>::iterator si = Syms.begin(),
    ...
    uint32_t Value;
    if (error((*si)->getName(Name))) continue;
    if (Name == "$d" || Name == "$a" || Name == "$t") continue;
    if (error((*si)->getAddress(Addr))) continue;
    ...
    if (Addr == object::UnknownAddressOrSize) Addr = 0;
    if (Size == object::UnknownAddressOrSize) Size = 0;
    char GlobLoc = ' ';
    if (Type != object::SymbolRef::ST_Unknown) GlobLoc = Global ? 'g' : 'l';
    char Debug = (Type == object::SymbolRef::ST_Debug ||
...
static void dumpCOFFSymbols(const object::COFFObjectFile *coff,
  ...
  /* AJG: I think this could read the complete section (I have not proven it to
   myself yet error_code ec = coff->getRvaPtr(Address, Res);
  ...
  /*
    ...
    for (object::delay_import_directory_iterator didi =
    coff->delay_import_directory_begin(); didi !=
    coff->delay_import_directory_end(); ++didi) { for
    (object::imported_symbol_iterator isi = didi->imported_symbol_begin(); isi
    != didi->imported_symbol_end(); ++isi) { StringRef symName; uint16_t
    ordinal; isi->getSymbolName(symName); isi->getOrdinal(ordinal); outs() <<
    "Ordinal: " << ordinal << "\t"; outs() << "SYMNAME: " << symName << "\n";
      }
    }
...
static void runSaveCommand(std::vector<std::string> &CommandLine) {
  ...
  std::error_code ErrorInfo;
  raw_fd_ostream FOut(CommandLine[1], ErrorInfo, sys::fs::OpenFlags::F_RW);
...
static void runDumpCommand(std::vector<std::string> &CommandLine) {
  ...
  for (std::size_t Index = Address, end = BaseAddr + Contents.size();
    ...
    for (std::size_t i = 0; i < 16 && Index + i < end; ++i) {
      if (std::isprint(
              static_cast<unsigned char>(Contents[Index - BaseAddr + i]) &
              0xFF))
...
static void initializeCommands() {
  ...
  CommandParser.registerCommand("load-binary", &runLoadBinaryCommand);
...
int main(int argc, char *argv[]) {
  ...
  if (std::error_code Err = loadBinary(InputBinaryFileName.getValue())) {
